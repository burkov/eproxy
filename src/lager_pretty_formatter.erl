...
output(module, Msg) ->
  ...
  io_lib:format("~-12s", [make_printable(get_metadata(module, Metadata, <<"ERTS">>))]);

output(line, Msg) ->
  case get_metadata(line, lager_msg:metadata(Msg)) of
    undefined -> "     ";
    LinoNo -> io_lib:format(":~-4s", [make_printable(LinoNo)])
  end;
